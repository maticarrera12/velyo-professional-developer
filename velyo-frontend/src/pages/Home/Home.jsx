...
const Home = () => {
  ...
  const { categories, getAllCategories, isLoading: isLoadingCategories, error: errorCategories } = useCategory();
  const {
    accommodations,
  ...
  const handleOnSubmmit = (e) => {
    ...
    dateRange &&
      dateRange.length > 0 &&
      (url += `checkIn=${dateRange[0].format("YYYY-MM-DD")}&checkOut=${dateRange[1].format("YYYY-MM-DD")}`);
    navigate(url);
  };
  ...
  useEffect(() => {
    if (accommodations && accommodations.length > 0) {
      ...
      const options = Array.from(uniquePlaces.values())
    ...
    <main className="home-main">
      ...
      <section className="home-recommendation-section">
        ...
        {isLoadingAccommodations ? (
          ...
          <Empty description="Ocurrió un error obteniendo los alojamientos." />
        ) : successAccommodation && randomAccommodations.length > 0 ? (
          <div className="recommendation-section-container">
            {randomAccommodations.map((accommodation) => (
