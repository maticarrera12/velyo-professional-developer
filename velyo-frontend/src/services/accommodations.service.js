...
const accommodationService = {
  ...
  getAccommodations: async (page, size) => {
    try {
      const response = await api.get(
        `/accommodations?page=${page}&size=${size}`
      );
      return response.data;
    } catch {
      throw new Error(
        "No se pudieron obtener los alojamientos. Por favor, intente nuevamente."
      );
    }
  },
  ...
  searchAccommodations: async (categoryIds, searchTerm, checkIn, checkOut) => {
    ...
    if (searchTerm) categoryIds += `&searchTerm=${searchTerm}`;
    if (checkIn && checkOut)
      categoryIds += `&checkIn=${checkIn}&checkOut=${checkOut}`;
  ...
  getAccommodation: async (id) => {
    try {
      ...
      throw new Error(
        "No se pudo obtener el alojamiento. Por favor, intente nuevamente."
      );
    }
  },
  deleteAccommodation: async (id, token) => {
    try {
      ...
      console.log(error);
      throw new Error(
        "No se pudo eliminar el alojamiento. Por favor, intente nuevamente."
      );
    }
  },
  createAccommodation: async (data, token) => {
    ...
    return api
      ...
      .catch((error) => {
        console.log(error);
        throw new Error(
          "No se pudo crear el alojamiento. Por favor, intente nuevamente."
        );
      });
  },
  editAccommodation: async (data, token) => {
    ...
    data.imagesToDelete.length > 0 &&
      formData.append(
        "imagesToDelete",
        new Blob([JSON.stringify(data.imagesToDelete)], {
          type: "application/json",
        })
      );

    return api
      .put("/accommodations", formData, {
        headers: {
          Accept: "application/json",
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
